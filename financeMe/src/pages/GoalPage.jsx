...
import { API_URL, getGoals, addGoal, getLoans, addLoan } from "../utils/api";
...
const GoalDashboard = () => {
  ...
  const handleGoalAnalysis = async (goal) => {
    try {
      const response = await fetch(`${API_URL}/goals/long-term/analyze`, {
        method: "POST",
        headers: {
  ...
  const handleLoanAnalysis = async (loan) => {
    try {
      console.log("running");
      const response = await fetch(`${API_URL}/loan/analyze`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
